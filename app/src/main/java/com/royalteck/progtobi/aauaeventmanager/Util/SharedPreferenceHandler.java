...
public class SharedPreferenceHandler {
    ...
    public static void saveDeveloperRecords(Context context,EventsResponse eventsResponse){
        Gson gson = new Gson();
        String devRecords = gson.toJson(eventsResponse);
        saveSharedPreferenceString(context,DEV_KEY,devRecords);
    }

    public static EventsResponse fetchDeveloperRecords(Context context) {
        SharedPreferences settings = context.getSharedPreferences(PREF_FILE, 0);
        String records= settings.getString(DEV_KEY, "{}");
        return new Gson().fromJson(records,EventsResponse.class);
    }
